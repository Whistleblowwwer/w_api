...
export const commentMetaData = async (comments, userID) => {
    ...
    const repliesCountForComments = await Comment.findAll({
        attributes: [
            "_id_parent",
            [
                Sequelize.fn("COUNT", Sequelize.col("_id_parent")),
                "repliesCount",
            ],
        ],
    ...
    const repliesMetaDataObject = repliesCountForComments.reduce(
        (acc, reply) => {
...
export const createCommentInteractionsDTO = async (comments, userID) => {
    ...
    const { likesMetaDataForComments, repliesCountForComments } =
        await commentMetaData(comments, userID);
