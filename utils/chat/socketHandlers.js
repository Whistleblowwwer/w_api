...
export const leaveConversation = (socket, conversationData) => {
    try {
        ...
        if (socket.roomsSet.has(_id_room)) {
            ...
            console.log(
                `User ${socket.user._id_user} is not in room ${_id_room}, so cannot leave it`
            );
        }
    } catch (error) {
        console.error(
            "An error occurred while trying to leave the conversation",
            error
        );
        socket.emit(
            "error",
            "An unexpected error occurred while leaving the conversation"
        );
    }
};
